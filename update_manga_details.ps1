...
if (-not (Test-Path -Path "./images/backgrounds")) {
    Log-Message "Creating backgrounds directory..."
    New-Item -Path "./images/backgrounds" -ItemType Directory -Force | Out-Null
}

# Read the template file
$templateContent = Get-Content -Path "./manga/template-manga.html" -Raw
...
"@
        ...
        if ($content -match '<ul class="chapter-list">.*?\[CHAPTER_' -or $content -match '<ul class="chapter-list">.*?</ul>\s*<div class="load-more">') {
        ...
        $backgroundPath = "./images/backgrounds/$mangaSlug-bg.jpg"
        if (-not (Test-Path -Path $backgroundPath)) {
            # Try to copy from covers
            $coverPath = "./images/covers/$mangaSlug.jpg"
            if (Test-Path -Path $coverPath) {
                Log-Message "  Copying cover to background for $mangaName..."
                Copy-Item -Path $coverPath -Destination $backgroundPath
            } else {
                Log-Message "  No cover found for $mangaName. Using placeholder."
            }
        }
...
Log-Message "All manga detail pages have been updated with the new design!"