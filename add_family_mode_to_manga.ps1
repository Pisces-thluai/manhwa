...
# Now handle files in the adult/manga directory
$adultMangaDir = Join-Path -Path (Get-Location) -ChildPath "adult/manga"
if (Test-Path $adultMangaDir) {
    $adultMangaFiles = Get-ChildItem -Path $adultMangaDir -Filter "*.html"
    
    Log-Message "Found $($adultMangaFiles.Count) manga files in adult directory to process."
    
    foreach ($file in $adultMangaFiles) {
        Log-Message "  Processing adult/manga/$($file.Name)..."
...
'@
        ...
        Log-Message "    Updated adult/manga/$($file.Name) with search features."
    }
}

Log-Message "All manga files have been updated with search features!" 